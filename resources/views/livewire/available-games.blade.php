<section class="w-full max-w-4xl mx-auto py-12 md:py-20" @auth wire:init="loadGames" @endauth>
            ...
            <div>
                <div class="text-center">
                    ...
                    @if(auth()->user()->role === 'ADMIN')
                        <h2 class="mt-2 text-lg font-medium text-gray-300">Create a New Game</h2>
                    @else
                        <h2 class="mt-2 text-lg font-medium text-gray-300">Join a Game</h2>
                    @endif
                </div>
                @if(auth()->user()->role === 'ADMIN')
                ...
                <ul role="list" class="mt-4 border-t border-b border-gray-200 divide-y divide-gray-200">
                        ...
                        <li class="py-4 flex items-center justify-between space-x-3">
                            <div class="min-w-0 flex-1 flex items-center space-x-3">
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-medium text-gray-200 truncate">{{ $game->host->name }}'s Game</p>
                                    <p class="text-sm font-medium text-gray-300 truncate">Created on {{ $game->created_at->format('F j, Y') }}</p>
                                </div>
                            </div>
                    ...
                    @empty
                        <li class="py-4 flex items-center justify-between space-x-3">
                            <div class="min-w-0 flex-1 flex items-center space-x-3">
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-medium text-gray-200 truncate">No Games Available</p>
                                    <p class="text-sm font-medium text-gray-300 truncate">Please refresh</p>
                                </div>
                            </div>
                        </li>
                    @endforelse
    ...
    @else
        <a href="/login" class="relative block w-full border-2 border-gray-300 border-dashed rounded-lg p-12 text-center hover:border-custom-red focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-custom-red">
            <svg class="mx-auto h-12 w-12 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
            <span class="mt-2 block text-sm font-medium text-gray-200">
